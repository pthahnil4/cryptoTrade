...
- channel: 频道名，如 "mark-price-candle1m", "mark-price-candle5m"
- callback: 回调函数，处理接收到的K线数据

运行步骤：
1. 修改下方的产品ID和时间周期列表
2. 运行脚本：python demo42_标记价格K线WebSocket订阅.py
3. 观察实时K线数据
4. 按Ctrl+C停止订阅

API文档：
https://www.okx.com/docs-v5/zh/#websocket-api-business-mark-price-candlesticks-channel
...
# 对应的频道名：mark-price-candle1m, mark-price-candle3m, etc.
TIMEFRAMES = [
    "1m",   # 1分钟K线
    "5m",   # 5分钟K线
    "15m",  # 15分钟K线
...
def handle_kline_data(message: Dict[str, Any]):
    ...
    # 检查是否为K线数据
    if not channel.startswith('mark-price-candle'):
        print(f"[WARNING] 接收到非K线数据: {channel}")
        return
    
    # 提取时间周期
    timeframe = channel.replace('mark-price-candle', '')
    
    for kline in data_list:
        # K线数据格式: [ts, o, h, l, c, confirm, ...]
        if len(kline) >= 6:
            ...
            # 显示K线信息
            print(f"📊 [{time_str}] {inst_id} ({timeframe}) OHLC: {o}/{h}/{l}/{c} {status}{price_change}")
...
async def subscribe_mark_price_klines(inst_ids: List[str], timeframes: List[str]):
    """
    订阅标记价格K线
    ...
    try:
        ...
        for inst_id in inst_ids:
            for timeframe in timeframes:
                args.append({
                    "channel": f"mark-price-candle{timeframe}",
                    "instId": inst_id
                })
...
async def main():
    ...
    # 创建任务
    tasks = [
        asyncio.create_task(subscribe_mark_price_klines(INST_IDS, TIMEFRAMES)),
