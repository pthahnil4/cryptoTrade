...
"""
OKX API Demo 27 - 市价减仓平仓交易
============================

功能说明：
- 市价减仓交易（reduceOnly=true）
- 市价平仓交易（开平仓模式下的平仓操作）
- 处理订单返回结果
- 订单状态跟踪
...
INST_ID = "NEAR-USDT-SWAP"           # 交易对：NEAR永续合约
...
ORDER_TYPE = "market"               # 订单类型: market=市价单（固定）
SIZE = "0.1"                        # 操作数量 (张)
...
try:
    if OPERATION_MODE == 1:
        print(f"\n⚡ 正在执行市价减仓操作...")
        if USE_POS_SIDE:
            print(f"   市价减少 {SIZE}张 {INST_ID} {POS_SIDE}持仓...")
        else:
            print(f"   市价减少 {SIZE}张 {INST_ID} 持仓...")
    else:
        print(f"\n⚡ 正在执行市价平仓操作...")
        if USE_POS_SIDE:
...
try:
    ...
    if code == '0':
        ...
        if sCode == '0':
            if OPERATION_MODE == 1:
                print(f"\n🎉 市价减仓订单创建成功！")
                print(f"   订单已提交，市价单通常会立即成交...")
                
                # 市价减仓订单状态说明
                print(f"\n💡 市价减仓订单说明:")
                print(f"   - 市价单按当前市场最优价格立即执行")
                print(f"   - 该订单只会减少持仓，不会增加新仓位")
                print(f"   - 减仓数量不能超过当前持仓数量")
                print(f"   - 成交价格可能与预期有轻微差异")
                
            else:
                print(f"\n🎉 市价平仓订单创建成功！")
                print(f"   市价平仓订单已提交，应该立即成交...")
                
                # 市价平仓订单状态说明
                print(f"\n💡 市价平仓订单说明:")
                print(f"   - 市价单会立即按最优价格执行")
    ...
    if code == '0' and sCode == '0':
        ...
        print(f"\n3️⃣  市价单特点:")
        print(f"   - 市价单通常立即成交，无需撤销")
        print(f"   - 如需查看成交价格，请查询订单详情")
        print(f"   - 建议及时检查持仓和资金变化")
    ...
    if OPERATION_MODE == 1:
        ...
        print(f"   - 市价减仓按当前最优价格立即执行")
        print(f"   - 减仓数量不能超过当前持仓数量")
        print(f"   - 适合快速减仓和风险控制场景")
    else:
        print(f"   - 开平仓模式下，平仓单自动具有只减仓逻辑")
        print(f"   - 市价平仓能够快速退出持仓")
        print(f"   - side=sell + posSide=long = 平多仓")
        print(f"   - side=buy + posSide=short = 平空仓")
    
    print(f"\n⚠️  风险提醒:")
    print(f"   - 市价单成交价格可能与预期有差异")
    print(f"   - 在市场波动剧烈时要特别注意滑点")
...
print("\n📊 交易结果:")
print(f"   产品: {INST_ID}")
print(f"   操作: {'减仓' if OPERATION_MODE == 1 else '平仓'}")
print(f"   方向: {SIDE}")
if USE_POS_SIDE:
    print(f"   持仓方向: {POS_SIDE}")
print(f"   数量: {SIZE}张")
...
print(f"   交易模式: {TRADE_MODE}")
if REDUCE_ONLY:
    print(f"   只减仓: {'是' if REDUCE_ONLY else '否'}")
