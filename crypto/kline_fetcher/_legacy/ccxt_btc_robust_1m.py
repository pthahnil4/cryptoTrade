...
def parse_args():
    parser = argparse.ArgumentParser(description="Robust BTC OHLCV 1m fetch with dedup and gap backfill")
...
def default_save_path() -> str:
    ...
    filename = "btc_ohlcv_robust_1m_since_2022.csv"
    ...
    while retry_count < max_retries:
        try:
            ...
            if "rate limit" in msg or "throttle" in msg or "too many requests" in msg or "429" in msg:
                ...
                continue
            # 捕获500错误和其他服务器错误
            if "500" in msg or "502" in msg or "503" in msg or "504" in msg or "internal server error" in msg:
                retry_count += 1
...
def main():
    ...
    if last_ts is not None:
        ...
        last_dt = datetime.datetime.fromtimestamp(last_ts / 1000, datetime.UTC).isoformat()
        print(f"检测到已有数据 {len(ordered_ts)} 条，最后时间: {last_dt}")
        print(f"从断点续传: {datetime.datetime.fromtimestamp(start_ms / 1000, datetime.UTC).isoformat()}")
    elif args.resume and last_ts is not None:
        start_ms = last_ts + step_ms
    else:
        start_ms = int(args.since)
        print(f"从头开始获取: {datetime.datetime.fromtimestamp(start_ms / 1000, datetime.UTC).isoformat()}")
